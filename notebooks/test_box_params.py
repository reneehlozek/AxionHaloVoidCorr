...
handle = '1024Mpc_n2048'  # string to identify the run; used to set filenames
output_folder = '1024Mpc_n2048'   # /path/to/folder/ where output should be placed
# ========================================= #
...
nbins = 1024     # the number of grid cells per side of the box
...
tracer_file = 'Trial_PP_halo_catalog_1024Mpc_n2048.npy'     # /path/to/file with input data
tracer_file_type = 2  # 1 for FITS file, 2 for array in numpy pickle format (.npy), 3 for array in ASCII format
...
box_length = 696.32   # if is_box, the box side length in Mpc/h; else ignored
...
z_high_cut = 0.2    # higher redshift cut (ignored if not survey)
...
z_min = 0.01        # minimum redshift extent of the data
z_max = 0.05        # maximum redshift extent of the data
...
min_dens_cut = 1. #0.000001  # void minimum galaxy number density (in units of mean density) reqd to qualify
...
zobov_buffer = 0.06 #0.08  # fraction of box length overlap between sub-boxes
# -------------------------- #
